...
\item \textbf{MC-statistical uncertainty of the learned ratios.} ATLAS propagates it as one NP via the
  ...
  interference/morphing/weighted-MC).
\end{enumerate}
...
6 MC-stat of learned ratios & real (ATLAS) but a \emph{published correctness critique} (Shyamsundar, 2505.19156); leading candidate fix = analytic-frequentist \emph{wifi ensembles} (2506.00113) & hybrid & high\\
...
finite-MC uncertainty, so settle this before quoting a number (leading candidate replacement: the
analytic-frequentist \emph{wifi}-ensemble estimator, 2506.00113); and \#2 --- the $\sim$40-NP scale may force a
...
\bibitem{shyam} P.~Shyamsundar, \emph{Comment on ``An implementation of neural simulation-based inference for
  ...
  critique, not merely an under-validation. Bears on item~6.
\bibitem{wifi} S.~Benevedes, J.~Thaler, \emph{Frequentist Uncertainties on Neural Density Ratios with wifi
  Ensembles}, arXiv:2506.00113 (2025). Trains basis functions $f_i(x)$ and propagates the weight covariance
  \emph{analytically} (no bootstrap) to an asymptotic frequentist uncertainty band on a learned density ratio;
  empirically correct coverage on a q/g SBI example. Leading candidate estimator for item~6 --- addresses both the
  correctness critique \emph{and} the ensemble/bootstrap compute multiplier; per-event band also feeds the honest
  morphing-uncertainty of item~9/\#15. Untested on interference/morphing/weighted-MC.
