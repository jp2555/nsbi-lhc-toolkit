...
4 \kl-dependent (POI-coupled) theory & \emph{proposed only} --- no POI-coupled per-event morphing anywhere; off-shell structurally avoids it & \textbf{open} & high\\
...
\textbf{One challenge is conceptually open} (no literature to tackle it): \#4, the \kl-dependent / POI-coupled theory
morphing $w_\theta(x;\kl,\alpha_\theta)$. The standard factorised scheme keeps every NP weight \emph{POI-independent}
($w_m(x;\alpha_m)$), and off-shell avoids the problem entirely ($\mu$ does not couple to its theory band because its
signal \emph{shape} is a fixed mixture). Note \#4 (POI$\times$NP) is distinct from \#5 (NP$\times$NP): GOLLUM solves
the latter on a toy but does \emph{not} reach the former. \#3 (per-event systematics of a \emph{data-driven} density)
...
scaling gate; then \#3/\#1/\#5 closures; the upsides (a,b) are field-first demonstrations.
\end{takeaway}
...
(hence ``open''): the \emph{component-differential} theory bands (the LHC HH WG tabulates only the total
$+4/{-}18\%$), and any NSBI closure test of per-component theory morphing.
