...
Trailing \sref{\S n} point to the relevant numbered section below.}
...
\item \priZ\,\eff{L} \textbf{Canonical ABCDnn flow for QCD \& jet$\to\tau$ fakes} (yield/shape split, three-piece
...
yield; max stable dimension documented. \sref{\S4}
...
$<2\%$ \kl\ bias. \sref{\S4}
\item \priO\,\eff{L} \textbf{Quantify the binned-QCD/fakes vs fully-unbinned cost.} Asimov (\kl$=0,1,5$): full
...
\item \priO\,\eff{M} \textbf{Validate poly-exp interpolation (\kl\ scan $+$ $\pm1\sigma$ anchors) under limited MC.}
...
\begin{enumerate}[resume,leftmargin=2.6em]\small
\item \priZ\,\eff{M} \textbf{Port the \textsc{jax}+\textsc{iminuit} profile fit with the
...
unbinned-vs-binned width $<5\%$. \sref{\S1,\,\S6}
...
\item \priZ\,\eff{M} \textbf{Asimov \& closure: morphing bias under injected shifts and generator/Delphes
...
\item \priZ\,\eff{M} \textbf{Neyman construction \& coverage validation on MC-truth toys.} 1000$+$ toys; build the
belt at \kl$\in\{-1,0,1,3,5\}$. \dw empirical coverage $\ge67\%$, $|$bias$|<0.1$ at SM; any failure traced to a
specific NP/component. \sref{\S6}
\item \priO\,\eff{M} \textbf{MC-stat ensemble-spread nuisance: design, size, coverage, Barlow--Beeston
...
\item \priO\,\eff{S} \textbf{Interference-aware impact definition $+$ binned-NSBI fallback.}
...
\begin{enumerate}[resume,leftmargin=2.6em]\small
\item \priZ\,\eff{S} \textbf{Define \& validate the reference hypothesis $P_{\rm ref}$.} Compare SM /
...
$k_{2V}$ tail. \dw C2ST $p>0.05$ across the scan, $|\log r|<10$, $<5\%$ basis bias at extremes. \sref{\S1}
\item \priZ\,\eff{M} \textbf{Feature-ablation degeneracy-breaking: $\rho($\kl,JES/$\tau$ES$)$ and VIF vs feature
...
$\rho<0.7$, VIF$<1.5$; settle whether constituents/SVfit push $\rho<0.60$ or it plateaus. \sref{\S2,\,\S5}
...
\item \priO\,\eff{M} \textbf{Validate the LO$\to$NLO \kl-reweighting that generates the $R(x|$\kl$)$ sources.}
Direct-NLO vs 2D-LO-reweight vs dedicated-NLO-reweight; propagate the difference through an Asimov fit. \dw
NLO-reweight reproduces direct to $<5\%$/bin and $<0.1\sigma$ \kl\ bias; the LO-only cost quantified. \sref{\S2}
...
\begin{enumerate}[resume,leftmargin=2.6em]\small
\item \priO\,\eff{M} \textbf{Low-signal robustness: morphing calibration \& uncertainty honesty at
...
\item \priT\,\eff{L} \textbf{Reproducible Perlmutter pipeline for the $\sim$2000-NN morphing ensemble.}
