...
\kl-dependent \emph{rate} weight; then fit Asimov data generated with scale-varied \emph{shapes} using a
normalisation-only model and measure the \kl\ bias. \dw a smooth band reproducing the AN-25-103 Table-40 values
(incl.\ the $m_t$ sign flip); truncation bias quantified ($<0.05$ $\Rightarrow$ CMS-style normalisation-only is
justified). \textcolor{orange!75!black}{\emph{\small(Prelim.: the total-level band per \kl\ now exists ---
...
\kl\ coefficients reproduces automatically. \textbf{Gate refocus:} the C4 pilot is no longer ``can we handle THU
at all'' but (i) source the component-differential bands, and (ii) measure the \emph{truncation bias} --- fit
Asimov data generated with scale-varied \emph{shapes} using the normalisation-only model and measure the \kl\
bias: small $\Rightarrow$ the CMS truncation is justified and C4b is deprioritised; large $\Rightarrow$
per-component shape morphing becomes a genuine physics improvement NSBI can claim (folded into studies
\#5/\#19).
\end{takeaway}
