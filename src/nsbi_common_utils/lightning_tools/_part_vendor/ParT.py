# VENDORED from weaver-core (MIT License, Copyright (c) 2020 Huilin Qu)
#   source: https://github.com/hqucms/weaver-core/blob/main/weaver/nn/model/ParticleTransformer.py
# The ONLY change from upstream is replacing the `from weaver.utils.logger import
# _logger` import with a stdlib logging stub, so this file is torch-only (we do not
# depend on the weaver-core package — it pins uproot<5.2, which conflicts with this
# toolkit's uproot 5.7). Keep this header and the MIT terms intact.
...
import logging  # vendoring stub: replaces `from weaver.utils.logger import _logger`
_logger = logging.getLogger("ParT")
