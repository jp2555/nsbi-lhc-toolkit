...
Calibration Data Efficiency Not Yet Resolved}
\author{Working note --- \bbtt\ \kl\ NSBI project}
\date{16 July 2026 (rev.\ 14 August 2026: (P5), Sec.~\ref{sec:power}) \\[2pt]
...
from-scratch. \textbf{(iii)~Calibration data efficiency: underpowered, not null} --- the pre-registered
adoption metric, the closure left-shift, is undefined because neither fine-tuned nor
from-scratch reaches the gates at \emph{any} fraction; and the reduced-fraction ordering is
dominated by seed-to-seed training noise, which on these same 90 networks is
$13$--$160\times$ the corresponding AUC seed noise and persists at full statistics
(Sec.~\ref{sec:power}). A powered extension --- fifteen further seeds per cell at the five
reduced fractions, a paired test-set bootstrap, and the decision rule fixed in advance ---
is registered as (P5) in Sec.~\ref{sec:prereg}. In
...
\item \textbf{Ensemble metrics at $K{=}5$ carry no seed error bar}: each cell is a single
  ensemble. The paired Poisson test-set bootstrap (since implemented; see (P5)) supplies
  the test component, and the $K{=}20$ extension the seed component. The
  $\chi^2/\mathrm{ndf}$ comparison in Sec.~\ref{sec:closure}(b) remains the one $K{=}5$
  shape statement with a quantified significance.
